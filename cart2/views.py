...
def checkout_links_mp(request):
    ...
    links_validos = [l for l in links if l.get('link')]
    errores = []
    if not links_validos:
        # Si hay links None, mostrar el motivo si está en el objeto
        for l in links:
            if not l.get('link') and l.get('vendedor'):
                errores.append(f"Vendedor: {l['vendedor']} - Error: No se pudo generar el link de pago. Verificá credenciales MercadoPago.")
        if not errores:
            errores.append("No hay enlaces de pago disponibles. Verificá que los vendedores tengan MercadoPago vinculado.")
        return render(request, 'cart/checkout_links_mp.html', {'links': [], 'errores': errores})

    return render(request, 'cart/checkout_links_mp.html', {'links': links_validos, 'errores': errores})
